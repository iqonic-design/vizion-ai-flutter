...
import 'package:vizion_ai/utils/constants.dart';
...
class ContentGenController extends GetxController {
  ...
  RxBool isContentReady = false.obs; // ✅ ADDED
  ...
  Future<void> generateContent(String title) async {
    loginPremiumTesterHandler(
      () async {
        isContentReady(false); // ✅ Set false before generating
        ...
        TextCompletionsRepository().textCompletions(
          ...
          onStreamCreated: (subscription) {
            Get.to(() => GenerateResultScreen(title: title));
            isLoading(false);
          },
          onComplete: (p0) async {
            clearTextField();
            ...
            await generateTitle(prompt).then((value) => isContentReady(true));
          },
