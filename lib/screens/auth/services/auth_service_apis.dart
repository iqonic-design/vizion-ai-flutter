...
import 'package:vizion_ai/configs.dart';
import 'package:vizion_ai/screens/auth/services/social_logins.dart';
import 'package:vizion_ai/screens/dashboard/dashboard_controller.dart';
  ...
  static Future<void> getAppConfigurations({bool forceConfigSync = false}) async {
    ...
    if (forceConfigSync || currentTimeStamp.isAfter(fiveMinutesLater)) {
      try {
        ...
        if (appConfigs.value.cutoutproKey.trim().isEmpty) {
          appConfigs.value.cutoutproKey = cutoutProkeys[Random.secure().nextInt(cutoutProkeys.length - 2)];
        }

        if (appConfigs.value.isInAppPurchaseEnable.getBoolInt()) {
          isInAppPurchaseEnable(appConfigs.value.isInAppPurchaseEnable.getBoolInt());
