...
import 'package:vizion_ai/screens/home/model/home_detail_res.dart';
import 'package:vizion_ai/screens/home/services/history_api_service.dart';
import 'package:vizion_ai/screens/subscription_plan/models/subscription_history_response.dart';
...
class HomeController extends GetxController {
  ...
  Future<void> init() async {
    //info = await getPackageInfo(); // fetch package info

    getDashboardDetail();

    /// auto-update check
    //_checkAndShowDialog(getContext, getBoolAsync(AutoUpdateConst.isAutoUpdateOn));
  }
  ...
  void syncRevenueCatData(InAppPurchaseService inAppPurchaseService) {
  ...
  Future<Package?> getSelectedPlanFromRevenueCat() async {
    if (revenueCatOfferings != null && revenueCatOfferings!.current != null && revenueCatOfferings!.current!.availablePackages.isNotEmpty) {
      int index = revenueCatOfferings!.current!.availablePackages.indexWhere((element) => element.storeProduct.identifier == (isIOS ? selectedPlan.value.appStoreIdentifier : selectedPlan.value.playStoreIdentifier));
      if (index > -1) {
  ...
  //       androidPackageName: 'com.iqonic.vizionai',
  //       iosBundleId: 'com.vizion.ai',
  //       minVersion: info.versionCode,
  //     ),
  //     uiOptions: PlayxUpdateUIOptions(
  //       displayType: PlayxUpdateDisplayType.dialog,
  //       title: (info) => 'Update to v${info.newVersion} available',
  //       titleTextStyle: primaryTextStyle(),
  //       description: (info) => 'A new version is available. Please update to continue.',
  ...
  //   result.when(
  //     success: (isShown) {
  //       debugPrint(isShown ? 'Update prompt displayed' : 'No update needed or dismissed.');
  //     },
  //     error: (error) {
