...
import 'package:vizion_ai/components/subscription_restore_component.dart';
import 'package:vizion_ai/screens/home/home_controller.dart';
import 'package:vizion_ai/screens/subscription_plan/models/subscription_plan_req.dart';
import 'package:vizion_ai/screens/subscription_plan/services/plan_services_api.dart';
  ...
  Future<void> init() async {
    try {
      ...
      if (apiKey.isNotEmpty) {
        PurchasesConfiguration configuration = PurchasesConfiguration(apiKey)..appUserID = loginUserData.value.email;
        // ..observerMode = false;
...
//
  ...
  Future<void> startPurchase({
    ...
    if (!getBoolAsync(SharedPreferenceConst.HAS_IN_APP_SDK_INITIALISE_AT_LEASE_ONCE)) {
      ...
      await Purchases.purchasePackage(selectedRevenueCatPackage, googleProductChangeInfo: userCurrentSubscription.value.activeRevenueCatIdentifier.isNotEmpty ? GoogleProductChangeInfo(userCurrentSubscription.value.identifier) : null).then(
        (value) {
          toast(locale.value.waitForAWhile);
  ...
  Future<void> saveSubscription() async {
    ...
    final customerData = await getCustomerInfo();
    if (homeController.dashboardData.value.subscriptionPlan
        .validate()
        .any((element) => element.appStoreIdentifier == customerData.activeSubscriptions.validate().first || element.playStoreIdentifier == customerData.activeSubscriptions.validate().first)) {
      homeController.selectedPlan(homeController.dashboardData.value.subscriptionPlan
          .validate()
          .firstWhere((element) => element.appStoreIdentifier == customerData.activeSubscriptions.validate().first || element.playStoreIdentifier == customerData.activeSubscriptions.validate().first));
    }
