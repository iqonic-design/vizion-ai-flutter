...
const defaultImage = ref('/img/avatar/avatar.png');
...
const validationSchema = yup.object({
  ...
  mobile: yup.string()
        ...
        .test('is-valid-phone', 'Only numbers are allowed in phone number with country code', function(value) {
          const cleanedValue = value ? value.trim() : '';
          
          
          if (!cleanedValue) return false;

          const hasInvalidCharacters = /[^\d\s+]/.test(cleanedValue); 
          const hasValidStart = cleanedValue.startsWith('+');
          const hasTrailingAlphabets = /[a-zA-Z]+$/.test(cleanedValue); 
          const minLength = 7; 
          const maxLength = 15; 

          return (
            !hasInvalidCharacters && 
            !hasTrailingAlphabets && 
            hasValidStart && 
            cleanedValue.replace(/\s+/g, '').length >= minLength &&
            cleanedValue.replace(/\s+/g, '').length <= maxLength
          );
        }),
    });
