...
class PlanController extends Controller
{
    ...
    public function index()
    {
        ...
        $is_in_app_purchase_enable = Setting::get('is_in_app_purchase_enable', 0);

        return view('subscriptions::backend.plan.index', compact('module_action', 'module_name', 'columns', 'customefield', 'is_in_app_purchase_enable'));
    }
    ...
    public function store(PlanRequest $request)
    {
        ...
        $duplicatePlan = Plan::where('name', $request->name)->first();
        if ($duplicatePlan) {
            return response()->json([
                'message' => "Plan with name '{$request->name}' already exists!",
                'status' => false
            ], 400);
        }
    ...
    public function update(PlanRequest $request, $id)
    {
        ...
        $duplicatePlan = Plan::where('name', $request->name)->where('id', '!=', $id)->first();
        if ($duplicatePlan) {
            return response()->json([
                'message' => "Plan with name '{$request->name}' already exists!",
                'status' => false
