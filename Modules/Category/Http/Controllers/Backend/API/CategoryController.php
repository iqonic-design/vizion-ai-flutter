...
class CategoryController extends Controller
{
    ...
    public function categoryList(Request $request)
    {
        ...
        $category = Category::where('status',1)->with('systemservice');
        ...
        if($request->has('system_service')&& $request->system_service != ''){

            $category = $category->where('type',$request->system_service);
        }
